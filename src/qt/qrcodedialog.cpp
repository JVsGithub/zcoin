...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("ZCS")
    updateDisplayUnit();
}
...
{
    QString ret = QString("zcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ZCS unit input in lnReqAmount, we generate the URI with ZCS as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ZCS, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
